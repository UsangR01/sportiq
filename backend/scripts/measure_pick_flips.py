...
WHAT IS MEASURED. For each settled fixture, the pick is RECONSTRUCTED at kickoff minus 2, 6 and
12 hours and compared against its final pre-kickoff state:
...
Two different changes are counted separately, because they matter differently to a user:

    MARKET FLIP      the recommended BET changed (over 1.5 -> 1X). This is the disruptive one:
                     a user who acted on the earlier card now holds a different bet than the
                     app shows.
    PROBABILITY MOVE the same bet, a different number. Ordinary and usually an improvement --
                     odds landing is new information, not noise.
...
ranking, so the measurement cannot drift away from what the product actually shows.
    ...
    PYTHONPATH=. python scripts/measure_pick_flips.py --sport football --hours 2 6 12 24
"""
...
async def main() -> None:
    ...
    async with async_session_factory() as db:
        ...
        sport_by = {f.id: s for f, s, _ in rows}
        league_by = {f.id: le for f, _, le in rows}
        ...
        for o in (await db.execute(select(Odds).where(Odds.fixture_id.in_(ids)))).scalars():
            odds_by_fixture[o.fixture_id].append(o)

        preds_by_fixture = defaultdict(list)
        for p in (
            await db.execute(select(Prediction).where(Prediction.fixture_id.in_(ids)))
        ).scalars():
            preds_by_fixture[p.fixture_id].append(p)

        corners_reference = await bulk_corners_reference(db, fixtures)

    # 0.60 is the mobile default min_probability. A pick crossing it is what the user actually
    # reported for the WNBA card ("at 59% it did not show; by morning it was 66%") -- NOT a
    # market flip, which is why it is counted separately. A freeze on the recommended market
    # would not have touched that case at all.
    THRESHOLD = 0.60
    stats = {
    ...
    for fixture in fixtures:
        ...
        final = _pick_as_of(
            latest_before(kickoff),
            odds_rows,
            kickoff,
            sport_by[fixture.id],
            league_by[fixture.id],
            corners_reference.get(fixture.id),
        )
        for hours in args.hours:
            moment = kickoff - timedelta(hours=hours)
            earlier = _pick_as_of(
                latest_before(moment),
                odds_rows,
                moment,
                sport_by[fixture.id],
                league_by[fixture.id],
            ...
            if _identity(earlier) != _identity(final):
                ...
                if len(examples[hours]) < 6:
                    examples[hours].append(
                        f"{sport_by[fixture.id]}/{league_by[fixture.id]}: "
