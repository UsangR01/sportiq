...
~~Push notifications aren't fully end-to-end deliverable yet: no `EXPO_ACCESS_TOKEN` is provisioned and this project has never been linked to a real EAS project~~ — **the EAS half of that is STALE and was repeated for weeks after it stopped being true.** Verified 2026-08-11: `mobile/eas.json` exists with development/preview/production profiles, `extra.eas.projectId` is set (`ce90c4de-…`), the EAS account is `usangr01`, and a real internal-distribution APK was built on 2026-08-05 (SDK 57, build `813c8ce0-…`). Only the `EXPO_ACCESS_TOKEN` part still stands. `getExpoPushTokenAsync` can't mint a token — confirmed live on Android (blocked earlier by the Expo Go restriction above) and not yet tried on iOS/web. Same "correct code, no live credential yet" status as RotoWire/BallDontLie injuries.
...
- ~~Mobile build/release: EAS Build/Update/Submit — no `eas.json`/EAS project set up yet~~ — **WRONG as of 2026-08-11, corrected rather than deleted because the wrong claim is what got repeated.** `mobile/eas.json` is real (development/preview/production), the project is linked (`ce90c4de-…`, account `usangr01`), and a real APK was built 2026-08-05. Two live gaps remain: **`eas.json`'s preview and production profiles point `EXPO_PUBLIC_API_URL` at `https://sportpiq-api.onrender.com`, which has NO server behind it** (confirmed: Render's edge returns `x-render-routing: no-server`), so any preview/production build talks to a backend that does not exist; and no Sentry `organization`/`project` is configured, so those builds upload no source maps and their stack traces would be minified.
